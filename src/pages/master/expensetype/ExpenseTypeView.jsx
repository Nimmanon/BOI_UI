...
const ExpenseTypeView = () => {
  ...
  const getData = () => {
    if (params.id === undefined || params.id === null || params.id === 0)
      return;
    APIService.getById("ExpenseType/GetById", params.id)
      .then((res) => {
        setData(res.data);
      })
  ...
  const handledDeleteItem = () => {
    ...
    APIService.Post("ExpenseType/Delete", credential)
      .then((res) => {
  ...
  const handleSaveClick = (data) => {
    ...
    APIService.Put("ExpenseType/Put", data)
      .then((res) => {
        if (res.status !== 200) return;
        handleBackClick();
  ...
  return (
    <>
      <FormTitle action={"VIEW"} name={"ExpenseType"} />
